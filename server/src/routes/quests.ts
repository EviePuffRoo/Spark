...
questsRouter.get("/:id", async (req, res) => {
  const memberWorldIds = await getMemberWorldIds(req.userId!);
  const row = await prisma.questHook.findFirst({ where: { id: req.params.id, ...visibleEntityWhere(req.userId!, memberWorldIds) } });
...
questsRouter.post("/", async (req, res) => {
  ...
  if (typeof prerequisiteQuestId === "string") {
    const memberWorldIds = await getMemberWorldIds(req.userId!);
    const prereq = await prisma.questHook.findFirst({ where: { id: prerequisiteQuestId, ...visibleEntityWhere(req.userId!, memberWorldIds) } });
...
questsRouter.patch("/:id", async (req, res) => {
  ...
  if ("prerequisiteQuestId" in body) {
    if (typeof body.prerequisiteQuestId === "string") {
      ...
      const memberWorldIds = await getMemberWorldIds(req.userId!);
      const prereq = await prisma.questHook.findFirst({ where: { id: body.prerequisiteQuestId, ...visibleEntityWhere(req.userId!, memberWorldIds) } });
      if (!prereq) return res.status(403).json({ error: "You don't have access to this quest" });
